...
assert list1 == list3
print("Items are matching in both the pages", end ="\n")

# Addition of total
individualAmount = driver.find_elements_by_xpath("//tr/td[5]/p")
total = 0
for amt in individualAmount:
    total = total + float(amt.text)

print('Total amount of individual item is: ', total)

# ORIGINAL AMOUNT
originalAmount = driver.find_element_by_class_name("discountAmt").text

assert total == float(originalAmount)

print("Total {} and Total Amount {} is matching.".format(total, float(originalAmount)))
driver.close()